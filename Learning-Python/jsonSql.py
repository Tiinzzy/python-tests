...
for entry in json_data:
    ...
    if role_id is None:
        cur.execute('''INSERT INTO Role (title)
        VALUES ( ? )''', (role, ))
...
data = cur.execute('''SELECT User.name,Course.title, Member.role FROM 
...
num = cur.execute('''SELECT 'XYZZY' || hex(User.name || Course.title || Member.role ) AS X FROM 
    User JOIN Member JOIN Course 
    ON User.id = Member.user_id AND Member.course_id = Course.id
    ORDER BY X LIMIT 1;''')    
